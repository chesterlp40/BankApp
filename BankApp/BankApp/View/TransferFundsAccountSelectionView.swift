...
struct TransferFundsAccountSelectionView: View {
    ...
    var body: some View {
        VStack(
            ...
            Button(
                "From \(self.transferFundsVM.fromAccountType)"
            ) {
            ...
            Button(
                "To \(self.transferFundsVM.toAccountType)"
            ) {
                ...
                .foregroundColor(Color.white)
                .opacity(
                    self.transferFundsVM.fromAccount != nil ? 1.0 : 0.5
                )
                .disabled(
                    self.transferFundsVM.fromAccount == nil
                )
            
            TextField(
                "Amount",
                text: self.$transferFundsVM.amount
            )
